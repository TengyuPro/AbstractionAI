...
function loadTaskFromFile(e) {
    ...
    reader.onload = function(e) {
        ...
        try {
            ...
            test = contents['test'];
            infoMsg('Task loaded successfully!');
        } catch (e) {
...
function createResultDisplayArea() {
    var matrixInput = $(`#result_input`).val();
    var matrix = JSON.parse(matrixInput);

    try {
        var displayArea = $('#result_grid_display');
        displayArea.empty();
        var gridObject = convertSerializedGridToGridObject(matrix);
        fillJqGridWithData(displayArea, gridObject);
        fitCellsToContainer(displayArea, gridObject.height, gridObject.width, 400, 400);
    } catch (e) {
        errorMsg('Invalid matrix format: ' + e.message);
    }
}
